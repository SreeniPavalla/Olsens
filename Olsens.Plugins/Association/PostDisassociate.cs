...
{
    public class PostDisassociateWrapper : PluginHelper
    {
        /// <summary>
        /// Triggers on PostDisassociate of Funeral with Bearers, Conductors & Hearses. 
        /// Retrieves all remaining associated Bearers and stores Bearer names ";" separated in string field- Bearer
        /// Retrieves all remaining associated Conductors and stores Conductor names ";" separated in string field- Conductor
        /// Retrieves all remaining associated Hearses and stores Hearse names ";" separated in string field- Hearse
        /// </summary>
        /// <param name="unsecConfig"></param>
        /// <param name="securestring"></param>

        public PostDisassociateWrapper(string unsecConfig, string secureString) : base(unsecConfig, secureString) { }

        protected override void Execute()
        {
            try
            {
                if (Context.MessageName.ToLower() != "disassociate" || !Context.InputParameters.Contains("Target")) return;

                AppendLog("PostDisassociate - Plugin Excecution is Started.");
                ...
                if (relationship.SchemaName == "ols_opportunity_ols_bearer")
                    PostDisassociateBearer(target.Id);

                else if (relationship.SchemaName == "ols_opportunity_ols_conductor")
                    PostDisassociateConductor(target.Id);

                else if (relationship.SchemaName == "ols_opportunity_ols_hearse")
                    PostDisassociateHearse(target.Id);
                AppendLog("PostDisassociate - Plugin Excecution is Completed.");
            }
            catch (Exception ex)
            {
        ...
        public void PostDisassociateBearer(Guid Id) //RelatedEntities = new_bearer, Target = opportunity
        {
            try
            {
                AppendLog("PostDisassociateBearer method strated.");
                var bearersList = new StringBuilder();
                var bearers = GetDisassociatedBearers(Id);
                ...
                AppendLog("PostDisassociateBearer method completed.");
            }
            catch (Exception ex) { throw ex; }
        }
        public EntityCollection GetDisassociatedBearers(Guid id)
        {
            try
            {
        ...
        public void PostDisassociateConductor(Guid Id) //RelatedEntities = ols_conductor, Target = opportunity
        {
            try
            {
                AppendLog("PostDisassociateConductor method strated.");
                var conductorsList = new StringBuilder();
                var conductors = GetDisassociatedConductors(Id);
                ...
                AppendLog("PostDisassociateConductor method completed.");
            }
            catch (Exception ex) { throw ex; }
        }
        public EntityCollection GetDisassociatedConductors(Guid id)
        {
            try
            {
                string fetch = @"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='true'>
        ...
        public void PostDisassociateHearse(Guid Id) //RelatedEntities = ols_hearse, Target = opportunity
        {
            try
            {
                AppendLog("PostDisassociateHearse method strated.");
                var hearsesList = new StringBuilder();
                var hearsesColl = GetDisassociatedHearses(Id);
                ...
                AppendLog("PostDisassociateHearse method completed.");
            }
            catch (Exception ex) { throw ex; }
        }
        public EntityCollection GetDisassociatedHearses(Guid id)
        {
            try
            {
                string fetch = @"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='true'>
    ...
    public class PostDisassociate : IPlugin
    {
        string UnsecConfig = string.Empty;
        string SecureString = string.Empty;
        public PostDisassociate(string unsecConfig, string secureString)
        {
        ...
        {
            var pluginCode = new PostDisassociateWrapper(UnsecConfig, SecureString);
